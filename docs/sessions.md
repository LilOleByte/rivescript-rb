...
`set_uservar()` function, as well as recent reply history and private internal
state variables.
...
`get_uservars()` and `set_uservars()` are available to export and import the
variables yourself.
...
```ruby
# Provide the session_manager option to use a custom store instead of
# the default MemorySessionManager.
bot = RiveScript.new(
  session_manager: MyRedisSessions.new("localhost:6379")
)
```

To implement your own session manager, you should subclass the
`RiveScript::SessionManager` class and implement a compatible object.

## void set(string username, object data)

Set user variables for the user `username`. The `data` should be a Hash
...
A value of `nil` for a variable means it should be deleted from the
user's session store.

## string get(string username, string key) -> string

Retrieve a stored variable for a user.

If the user doesn't exist, this should return `nil`. If the user *does*
exist, but the key does not, this function should return the
string value `"undefined"`.

## object get_any(string username) -> object

Retrieve all stored user variables for the user `username`.

This should return a Hash of the key/value pairs you have stored for
the user. If the user doesn't exist, return `nil`.

## object get_all() -> object

Retrieve all variables about all users.

This should return a Hash that maps usernames to a Hash of their
...
`RiveScript#freeze_uservars()`

## void thaw(string username, string action)
...
Valid options for `action` reflect the usage of `rs.thaw_uservars()`:
...
It keeps all user variables in a Hash in memory and does not persist them
...
The RiveScript methods `get_uservars()` and `set_uservars()` are available to
export and import user variables as serializable objects so that your
